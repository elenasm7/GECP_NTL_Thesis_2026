...
def _open_nc_tile(year, tile):
    key = (int(year), tile)
    if key in nc_cache:
        return nc_cache[key]
    nc_path = f"{base_path}/Processed_data/Black_Marble_processed/corrected_NTL/{year}/nc/{tile}_{year}.nc"
...
output_base = f"{base_path}/Results/{region}_nc/"
post_base = f"{base_path}/Results/{region}_nc/postprocessing"
...
if os.path.exists(hazard_tif_path): 
    ...
    stacked_NTL = np.full((total_days_analysis, rows_aff, cols_aff), np.nan)  # Initialise
    analysis_start_date = sdate - timedelta(days=BUFFER_pre)
    analysis_end_date = edate + timedelta(days=BUFFER_post)

    #Stack relevant NTL data in year-sized blocks (faster than per-DOY reads)
    segments = []
    cur_date = analysis_start_date
    while cur_date <= analysis_end_date:
        year1 = cur_date.year
        year_end = datetime(year1, 12, 31)
        seg_end = min(year_end, analysis_end_date)
        doy_start = cur_date.timetuple().tm_yday
        doy_end = seg_end.timetuple().tm_yday
        t0 = (cur_date - analysis_start_date).days
        t1 = (seg_end - analysis_start_date).days + 1
        segments.append((year1, doy_start, doy_end, t0, t1))
        cur_date = seg_end + timedelta(days=1)
    ...
    for h in range(h_min, h_max + 1):
        for v in range(v_min, v_max + 1):
            ...
            # overlap of this tile with affected window
            ov_row_start = max(i_min, xmin)
            ov_row_end = min(i_max, xmax)
            ov_col_start = max(j_min, ymin)
            ov_col_end = min(j_max, ymax)
            if ov_row_start >= ov_row_end or ov_col_start >= ov_col_end:
                continue

            tile_row_start = ov_row_start - i_min
            tile_row_end = ov_row_end - i_min
            tile_col_start = ov_col_start - j_min
            tile_col_end = ov_col_end - j_min

            stack_row_start = ov_row_start - xmin
            stack_row_end = ov_row_end - xmin
            stack_col_start = ov_col_start - ymin
            stack_col_end = ov_col_end - ymin

            for (year1, doy_start, doy_end, t0, t1) in segments:
                nc_ds = _open_nc_tile(year1, tile_id)
                if nc_ds is None:
                    continue
                try:
                    ...
                    print(f"Cannot read NC NTL for {tile_id} year {year1} doy {doy_start}-{doy_end}")
                    continue
    ...
    preevent_avail = np.full((rows_aff, cols_aff), np.nan)
    ...
    if len(aff_rows_event2) > 0:
        ...
        for k in range(0, len(aff_rows_event2)):
            ...
            if BUFFER_pre > 0:
                preevent_avail[ii, jj] = (
                    np.sum(~np.isnan(pixel_ts[:BUFFER_pre])) / BUFFER_pre
                )
                    ...
                    if recov_idx_pers is None:   
                        ...
                        if store_for_print:
                            for t in range(0, total_days_analysis):
                                ntl_val = pixel_ts[t]
                                pre_mean = pre_event_mean[ii, jj]
                                pre_std = pre_event_std[ii, jj]
                                ...
                                if t >= idx_event_start:
                                    with np.errstate(divide='ignore', invalid='ignore'):
                                        DA_today = (pre_mean - ntl_val) / pre_std
                                ...
                                writer.writerow([
                                    row_idx_global, col_idx_global, ntl_val, t - BUFFER_pre + 1,
                                    DA_today, recovered, affected, affected_later, recovered_pers_val, affected_later_pers
                                ])
    
        # NEW: availability-adjusted durations and availability outputs
        save_raster(
            f"{post_base}/postimpact_availability/{region}_postimpact_avail.tif",
            postimpact_avail,
            haz_tif,
            im_row,
            im_col,
        ...
        save_raster(
            f"{post_base}/postworst_availability/{region}_postworst_avail.tif",
            postworst_avail,
            haz_tif,
            im_row,
            im_col,
        ...
        save_raster(
            f"{post_base}/preevent_availability/{region}_preevent_avail.tif",
            preevent_avail,
            haz_tif,
            im_row,
            im_col,
        ...
        for avail_th in AVAIL_THRESHOLDS:
            ...
            save_raster(
                f"{post_base}/recovery_duration/{region}_recov_dur_avail{pct_avail}.tif",
                recov_dur_avail,
                haz_tif,
                im_row,
                im_col,
            ...
            save_raster(
                f"{post_base}/impact_duration/{region}_impact_dur_avail{pct_avail}.tif",
                impact_dur_avail,
                haz_tif,
                im_row,
                im_col,
            ...
            save_raster(
                f"{post_base}/recovery_duration/{region}_recov_dur_pers_avail{pct_avail}.tif",
            ...
            save_raster(
                f"{post_base}/impact_duration/{region}_impact_dur_pers_avail{pct_avail}.tif",
