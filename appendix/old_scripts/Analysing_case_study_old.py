...
import sys
import os 
from osgeo import ogr # this seems like a gdal library, need to figure this out, might need new enve (with old python)
ogr.UseExceptions()
from datetime import datetime, timedelta
import warnings
import shapefile #I have no idea what this is, but maybe is a selfmade module???
import pandas as pd
import csv
...
def find_persistent_recovery(ts_recovered, start_idx, total_days, min_ratio, window, min_valid):
    ...
    for t in range(start_idx + window, total_days):
        # gets the dataframe and cuts it for the recovery window
        window_vals = ts_recovered[t - window : t]
        #selects where not null for filtering in the if.
...
#change these dates for the different events
...
def _parse_time_list(val):
    ...
    if isinstance(val, list):
        ...
        try:
            parsed = ast.literal_eval(val) #people seem to have caveats about this builtin potential rework with eval
            items = parsed if isinstance(parsed, list) else [parsed]
        except Exception:
            items = re.findall(r"\d{4}-\d{2}-\d{2}[^,\]]*", val)
    else:
        items = []
        #potential update pd.to_datetime(items, errors="coerce").tz_localize(None)
    return [pd.to_datetime(x).tz_localize(None) for x in items if x]

#%%

# being used for file naming and potentially for the # of std used
s_t = 3
...
if os.path.exists(hazard_tif_path): 
    ...
    #DAmax_adj and DPmax_adj will be between impact start and recovered, based on min_NTL_adj
    ...
    # get the cells where it's an outlier
    ...
    all_metric_names = [
        ...
        "recov_dur_adj",  "worst_day_adj", "min_NTL_adj", "DAmax_adj", "DPmax_adj", 
        "recov_dur_ap", "worst_day_ap", "min_NTL_ap", "DAmax_ap", "DPmax_ap" 
    ]

    #normal means worst impact during event duration

    #adjusted means worst impact before recovery, recovery day (and thus impact duration) stay the same,
    #but worst impact (hence also min NTL, DAmax and DPmax) and recovery time change
    #so:
        #recov_day_adj = recov_day
        #impact_dur_adj = impact_dur (incl. min_ntl, DAmax, DPmax)
        #new: recov_dur_adj, worst_day_adj, min_NTL_adj, DAmax_adj, DPmax_adj
    ...
    #Persistent adjusted means that the worst impact can again be before recovery. Recovery day (and impact duration)
    #stay the same as normal persistent, but worst impact (and min NTL, DAmax and DPmax) as well as recovery time change
        #recov_day_ap = recov_day_pers
        #impact_dur_ap = impact_dur_pers (incl. min_ntl, DAmax, DPmax)
        #new: worst_day_ap, recov_dur_ap, min_NTL_ap, DAmax_ap, DPmax_ap
    ...
    if len(aff_rows_event2) > 0:
        ...
        for k in range(0, len(aff_rows_event2)):
            ...
            if np.all(np.isnan(valid_range)):
                ...
                ts_recovered_adj = ts_recovered.copy()
                ...
                if len(recovered_indices[0]) >0: 
                    ...
                    if first_recovery_idx > idx_event_end: #only when recovery after event end
                        """Finding also worst dip if outside of duration but within (non-persistent) recovery period"""
                        recov_range = pixel_ts[idx_impact_start:first_recovery_idx] #timeseries during entire recovery period
                        #first recovery is after lowest dip within event duration

                        if np.all(np.isnan(recov_range)):
                            worst_day_adj[ii,jj] = NOT_ENOUGH_DATA 

                            if (
                                worst_day_adj[ii, jj] == NOT_ENOUGH_DATA and 
                                worst_day[ii, jj] in range(1, 11)
                            ):
                                print("ts_recovered:", ts_recovered)
                                print("valid_range:", valid_range)
                                print("idx_impact_start:", idx_impact_start)
                                print("first_recovery_idx:", first_recovery_idx)
                                print("event end:", idx_event_end)
                                print("recov_range:", recov_range)
                                
                                raise ValueError("Adjusted worst day was marked as NOT_ENOUGH_DATA despite valid worst_day")


                            DAmax_adj[ii, jj] = NOT_ENOUGH_DATA
                            DPmax_adj[ii, jj] = NOT_ENOUGH_DATA
                            recov_dur_adj[ii, jj] = NOT_ENOUGH_DATA
                            raise ValueError("this interval should always be longer than the original")

                        else:
                            lowest_dip_adj_rel = np.nanargmin(recov_range)  # index within the slice
                            lowest_dip_adj = lowest_dip_adj_rel + idx_impact_start  # index in full ts
                            worst_day_adj[ii,jj] = lowest_dip_adj - BUFFER_pre + 1
                            min_NTL_adj[ii, jj] = pixel_ts[lowest_dip_adj] #value of lowest dip before recov
                            min_adj = min_NTL_adj[ii, jj].copy()

                            with np.errstate(divide='ignore', invalid='ignore'):
                                DAadj = (mean - min_adj) / stdm
                                DPadj = (mean - min_adj) / mean

                                # Replace infinities and NaNs with np.nan
                                if not np.isfinite(DAadj) or abs(DAadj) > 999:
                                    DAadj = NODATA_VALUE
                                if not np.isfinite(DPadj) or abs(DPadj) > 999:
                                    DPadj = NODATA_VALUE

                            DAmax_adj[ii, jj] = DAadj
                            DPmax_adj[ii, jj] = DPadj

                            recov_dur_adj[ii, jj] = first_recovery_idx - lowest_dip_adj + 1

                            if recov_dur_adj[ii, jj] > recov_dur[ii, jj]:
                                print(f"Warning: Adjusted recovery duration is longer at pixel ({ii}, {jj})")
                                print(f"recov_day: {recov_day[ii, jj]}, worst_day: {worst_day[ii, jj]}, worst_day_adj: {worst_day_adj[ii, jj]}")
                                raise ValueError(f"Adjusted recovery duration ({recov_dur_adj[ii, jj]}) is longer than original ({recov_dur[ii, jj]}) at pixel ({ii}, {jj})")
                    ...
                    if recov_idx_pers is None:   
                        ...
                        """If persistent recovery found we can also check within that window for a more significant impact"""
                        recov_range_pers = pixel_ts[idx_impact_start:recov_idx_pers] #timeseries during entire recovery period
                        
                        if np.all(np.isnan(recov_range_pers)):
                            worst_day_ap[ii,jj] = NOT_ENOUGH_DATA
                            DAmax_ap[ii, jj] = NOT_ENOUGH_DATA
                            DPmax_ap[ii, jj] = NOT_ENOUGH_DATA
                            recov_dur_ap[ii, jj] = NOT_ENOUGH_DATA
                        else:
                            lowest_dip_ap_rel = np.nanargmin(recov_range_pers)  # index within the slice
                            lowest_dip_ap = lowest_dip_ap_rel + idx_impact_start  # index in full ts
                            worst_day_ap[ii,jj] = lowest_dip_ap - BUFFER_pre + 1
                            min_NTL_ap[ii, jj] = pixel_ts[lowest_dip_ap] #value of lowest dip before recov
                            
                            min_ap = min_NTL_ap[ii, jj].copy()

                            with np.errstate(divide='ignore', invalid='ignore'):
                                DAap = (mean - min_ap) / stdm
                                DPap = (mean - min_ap) / mean

                                # Replace infinities and NaNs with np.nan
                                if not np.isfinite(DAap) or abs(DAap) > 999:
                                    DAap = NODATA_VALUE
                                if not np.isfinite(DPap) or abs(DPap) > 999:
                                    DPap = NODATA_VALUE

                            DAmax_ap[ii, jj] = DAap
                            DPmax_ap[ii, jj] = DPap

                            recov_dur_ap[ii, jj] = recov_idx_pers - lowest_dip_ap + 1

                else:
                    for name in all_metric_names:
                        globals()[name][ii, jj] = RECOVERY_NOT_FOUND 

                """This is per day storage of results"""
                if store_for_print:
                    check_memory()  


                    #Added: calculate NTLi, DAi, DPi per pixel (store in csv)
                    for t in range(0, total_days_analysis):
                        ntl_val = pixel_ts[t]#value at time t

                        pre_mean = pre_event_mean[ii, jj] #one value
                        pre_std = pre_event_std[ii, jj] #one value
                        recovered = ts_recovered[t]
                        affected = aff[ii,jj]
                        affected_later = aff_later[ii, jj]
        ...
        metric_file_map = {
            ...
            "worst_day": "worst_day",
            "recov_dur_adj": "recovery_duration",
            "worst_day_adj": "worst_day",
            "min_NTL_adj": "event_min",
            "DAmax_adj": "DAmax_150",
            "DPmax_adj": "DPmax_150",
        }
        ...
        if np.any(recov_day_pers != NOT_ENOUGH_DATA):
            metric_file_map_pers = {
                    ...
                    "recov_dur_ap": "recovery_duration",
                    "worst_day_ap": "worst_day",
                    "min_NTL_ap": "event_min",
                    "DAmax_ap": "DAmax_150",
                    "DPmax_ap": "DPmax_150"
                }
